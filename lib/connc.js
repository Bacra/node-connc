...
function Processor()
{
	...
	self.on('newListener', function(event, handler)
	{
		if (event.substr(0, 6) == 'after:')
		{
			self._after_task.on(event.substr(6), handler);
		}
	});
...
_.extend(Processor.prototype,
{
	...
	toRenderData: function(data){return data},

	chooseRenderType: function(renderData)
	{
		debug('chooseRenderType:%o', renderData);
		return 'html';
	},
	renderThisType: function(type, renderData)
	{
		debug('renderThisType, type:%s, data:%o', type, renderData);
	},
	chooseErrorType: function(err)
	{
		debug('chooseErrorType:%s', err);
		return 'html';
	},
	renderThisError: function(type, err)
	{
		debug('renderThisError, type:%s, err:%o', type, err);
	},
});
